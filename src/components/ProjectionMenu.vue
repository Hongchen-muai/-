<template>
  <div ref="container" class="projection-selector" @pointerenter="enter" @pointerleave="leave" @focusout="focusOut" @keydown="keydown">
    <button ref="trigger" class="projection-trigger" :aria-label="`选择投影：${current.label}`" aria-haspopup="menu" :aria-expanded="opened" aria-controls="projection-drawer" @click="toggle">
      <span>{{ current.label }}<small>{{ current.labelEn }}</small></span>
    ...
    <div v-if="opened" id="projection-drawer" class="projection-drawer" :style="{ maxHeight: `${maxHeight}px` }">
      <div class="drawer-heading">投影类型 <span aria-hidden="true">{{ String(selectedIndex + 1).padStart(2, '0') }} / 05</span></div>
      <div class="projection-options" role="menu" aria-label="投影类型栏">
        <button v-for="(family, index) in options" :key="family.key" role="menuitemradio" :aria-checked="selected === family.key" :class="{ active: selected === family.key }" :style="itemStyle(index)" @click="select(family.key)">
          <span class="option-index" aria-hidden="true">{{ String(index + 1).padStart(2, '0') }}</span>
          <span class="option-name">{{ family.label }}<small>{{ family.labelEn }}</small></span>
        ...
        <button class="option-upcoming" role="menuitemradio" aria-checked="false" disabled>
          <span class="option-index" aria-hidden="true">05</span>
...
const selectedIndex = computed(() => props.options.findIndex(f => f.key === props.selected));
const current = computed(() => props.options[selectedIndex.value] || props.options[0]);
...
function select(key) { emit('select', key); close(true); }
...
async function keydown(event) {
  ...
  const index = event.key === 'Home' ? 0 : event.key === 'End' ? items.length - 1
    : active === -1 ? selectedIndex.value : (active + (event.key === 'ArrowDown' ? 1 : -1) + items.length) % items.length;
  items[index]?.focus();
}
...
.projection-trigger { display: flex; justify-content: space-between; align-items: center; gap: 12px; width: 100%; min-height: 54px; padding: 9px 14px; border: 1px solid #3c5145; border-radius: 3px; background: linear-gradient(135deg, #3c5b4c, #333); color: #fff; text-align: left; }
.projection-trigger > span { font-size: 14px; font-weight: 700; }
.projection-trigger small { display: block; margin-top: 2px; font: 10px/1.4 "Segoe UI", sans-serif; text-transform: uppercase; opacity: .8; }
...
@media (max-width: 600px) { .projection-drawer { width: 100%; max-width: none; } }
