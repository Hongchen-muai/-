...
export function projectionHelp(family, mode, params = {}) {
  ...
  const standard = family === 'planar'
    ...
    : parallel + (params.aspect === 'transverse' && family === 'cylinder' ? '横轴情况下，它属于旋转后的轴向坐标，不是地理纬线。' : '');
  return {
    ...
    aspect: '正轴的投影轴与地球极轴一致；横轴先变换球面坐标的极轴，不是把已经画好的二维地图旋转 90°。',
    ...
    compromise: family === 'planar' ? '此处为正射投影：采用平行投影，既非等角也非等面积，半球边缘径向压缩明显。'
      : '此处为等距投影：沿经线以及标准纬线的长度比例为 1，不保持任意方向或任意两点间的距离，也不是等角或等面积投影。',
...
export function annotationHelp(text, family) {
  ...
  if (/^[OPGM](=[OPGM])*$/.test(text)) return text.split('=').map(key => symbols[key]).join('\n')
    + (text.includes('=') ? '\n等号表示这些点在当前参数下位置重合。' : '');
  return 'φ 表示纬度，横轴情况下表示轴向纬度；c 表示距投影中心的球面角距。P 为用于说明映射关系的球面示例点。';
}
